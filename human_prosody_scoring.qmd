...
execute:
  ...
  warning: false
  message: false
bibliography: references-humanps.bib
...
# final ratings
data_ratings_final <- data_ratings %>% 
  filter(!is.na(rating)) %>% 
  filter(str_detect(rating, "No audio", negate = TRUE)) %>% 
  group_by(filename) %>% 
  mutate(n = n()) %>% 
  filter(n == 2) %>% 
  select(-n) %>% 
...
student_audiofiles <- data_ratings_final %>%
  group_by(core_id) %>% 
  count() %>% 
  ungroup()

rating_demos <- data_ratings_final %>% 
  left_join(dir_demos, by = c("filename" = "file")) %>% 
  select(core_id = core_id.x, filename, grade_core, student_gender, student_ethnicity, student_race, easycbm_sped, easycbm_ell) %>% 
  mutate(
...
After all usable audio files were selected the full sample included `r format(distinct(data_ratings_final, filename) %>% nrow(), big.mark=",")` audio files each rated twice. Of these from `r format(length(unique(data_ratings_final$core_id)), big.mark=",")` students (`r format(sum(data_ratings_final$grade_core == 2), big.mark=",")` in Grade 2, `r format(sum(data_ratings_final$grade_core == 3), big.mark=",")` in Grade 3, `r format(sum(data_ratings_final$grade_core == 4), big.mark=",")` in Grade 4). The number of audio files per student in the final sample ranged from `r min(student_audiofiles$n)` to `r max(student_audiofiles$n)`.
...
The results of the stratification yielded a sample of `r format(distinct(data_ratings_final, filename) %>% nrow(), big.mark=",")` audio files that was `r rating_strat[rating_strat$'Stratification Groups' == "EL & SWD", "percent"]`% (*n* = `r rating_strat[rating_strat$'Stratification Groups' == "EL & SWD", "n"]`) EL and SWD, `r rating_strat[rating_strat$'Stratification Groups' == "EL only", "percent"]`% (*n* = `r rating_strat[rating_strat$'Stratification Groups' == "EL only", "n"]`) EL only, `r rating_strat[rating_strat$'Stratification Groups' == "SWD only", "percent"]`% (*n* = `r rating_strat[rating_strat$'Stratification Groups' == "SWD only", "n"]`) SWD only, and `r rating_strat[rating_strat$'Stratification Groups' == "Not EL or SWD", "percent"]`% (*n* = `r rating_strat[rating_strat$'Stratification Groups' == "Not EL or SWD", "n"]`) neither EL or SWD. 

```{r demos}
student_demos_tbl <- student_demos %>%
  select(-core_id, -filename) %>% 
  tbl_summary(
    label = list(
      grade_core ~ "Grade",
...
To help draw clear differences between the four prosody levels across grades, parts of the Multi-Dimensional Fluency Scoring Guide (MFSG; @rasinski2009) were incorporated into the original NAEP rubric. 

```{r }
#| fig-cap: "From [Rasinski, Rikli, & Johnston (2009)](https://www.tandfonline.com/doi/pdf/10.1080/19388070802468715)"
#| out.extra: 'class="external"'
...
Human prosody raters were recruited and required to complete two [Training Sessions](https://jnese.github.io/coreprosody/human_prosody_scoring.html#training-development-implementation), and meet [Prosody Certification](https://jnese.github.io/coreprosody/human_prosody_scoring.html#prosody-certification) criterion. 
...
Of the `r nrow(data_traincert)` people who completed the [Trainings](https://jnese.github.io/coreprosody/human_prosody_scoring.html#training-development-implementation), `r sum(data_traincert$passed == "pass")` (`r round((sum(data_traincert$passed == "pass")/nrow(data_traincert))*100, 0)`%) met prosody certification, `r sum(data_traincert$passed == "fail")` (`r round((sum(data_traincert$passed == "fail")/nrow(data_traincert))*100, 0)`%) failed to meet certification, and `r sum(data_traincert$passed == "incomplete")` (`r round((sum(data_traincert$passed == "incomplete")/nrow(data_traincert))*100, 0)`%) did not complete the certification process.
...
data_raters <- read_csv(here::here("nopublish", "data_raterdatabase.csv")) %>% 
  filter(first != "dottie") %>% 
  select(-c(batches, files_scored))

data_raters <- data_ratings_final %>% 
  group_by(email) %>% 
  summarize(batches = n_distinct(batch),
            files_scored = n()) %>% 
  left_join(data_raters, by = "email")
...
* `r nrow(filter(data_raters, role == "school psychologist | social worker | counselor | behavior specialist | etc."))` (`r round(nrow(filter(data_raters, role == "school psychologist | social worker | counselor | behavior specialist | etc."))/nrow(data_raters)*100, 0)`%) reported as "school psychologist | social worker | counselor | behavior specialist | etc."

* `r nrow(filter(data_raters, role == "administrator | principal | district support"))` (`r round(nrow(filter(data_raters, role == "administrator | principal | district support"))/nrow(data_raters)*100, 0)`%) reported as "administrator | principal | district support"

* `r nrow(filter(data_raters, str_detect(role, "retired")))` (`r round(nrow(filter(data_raters, str_detect(role, "retired")))/nrow(data_raters)*100, 0)`%) were retired, and reported their role as special educators before retirement

* `r nrow(filter(data_raters, str_detect(role_contentarea, "ESOL")))` (`r round(nrow(filter(data_raters, str_detect(role_contentarea, "ESOL")))/nrow(data_raters)*100, 0)`%) an "other content area specialist" (i.e., ESOL)
...
In this manner, all `r format(nrow(distinct(data_ratings_final, filename)), big.mark=",")` audio recordings were rated by two different prosody raters from November 28, 2020 through February 8, 2021, for a total of `r format(nrow(data_ratings_final), big.mark=",")` human prosody scores.
