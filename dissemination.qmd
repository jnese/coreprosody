...
```{r}
embed_file_quarto <- function(path, text, filename = basename(path)) {
  data <- base64enc::base64encode(path)
  sprintf(
    '<a href="data:application/pdf;base64,%s" download="%s">%s</a>',
    data, filename, text
  )
}
```
...
**Leilani Sáez, Makayla Whitney, Joe Nese, Julie Alonzo, Rhonda Nese (2025). `r embed_file_quarto(path = here::here("nopublish", "doi_101002trtr2368.pdf"), text = "Reading prosody: A listening guide for teachers.")` _The Reading Teacher_, _78_, 243-251.**
</p> 

<font size="-0.5">

> Teachers can enhance students’ oral reading fluency by fine-tuning how they listen
to three reading prosody components: intonation, amplitude, and timing.
...
<p style="text-indent: -36px; padding-left: 36px; color:#6699AC;">
**Yihao Wang, Zhongdi Wu, Akihito Kamata, Joseph Nese, Vedant Nilabh, & Eric C. Larson (2025, April). `r embed_file_quarto(path = here::here("nopublish", "unified_model.pdf"), text = "A Unified Model for Oral Reading Fluency and Student Prosody.")` Presented at the IEEE International Conference on Acoustics, Speech and Signal Processing (IEEE ICASSP), Hyderabad, India.**
</p>
...
<p style="text-indent: -36px; padding-left: 36px; color:#6699AC;">
**Yihao Wang, Zhongdi Wu, Akihito Kamata, Joseph Nese, Vedant Nilabh, & Eric C. Larson (2024, April). `r embed_file_quarto(path = here::here("nopublish", "IEEE-ICASSP_2024.pdf"), text = "Improving Oral Reading Fluency Assessment Through Sub-Sequence Matching of Acoustic Word Embeddings.")` Presented at the IEEE International Conference on Acoustics, Speech and Signal Processing (IEEE ICASSP), Seoul, Korea.**
</p>

<font size="-0.5">

> Oral reading fluency assessment is a process where a student reads a passage aloud and is scored against words read correctly by a human listener. Current automatic reading fluency systems match these words read using speech recognition models trained with clean speech data from native adult speakers. This mismatch in training and deployment, compounded by numerous background noises from the classroom, means that student speech is often not correctly rec- ognized. This paper describes a deep learning model that employs text-to-speech and contrastive learning to create acoustic word em- beddings of student speech. This embedding is trained with unlabeled data of students reading known passages. Our model then uses sub-sequence matching in the acoustic embedding space to estimate words read correctly per minute, a common criterion in oral reading fluency. Our model’s words read correctly per minute is significantly closer to human listeners compared to systems that use automatic speech recognition only, reducing error of words correct per minute from 15.1 to 8.4, on average.
...
<p style="text-indent: -36px; padding-left: 36px; color:#6699AC;">
**Yusuf Kara, Akihito Kamata, Xin Qiao, Nelis Potgieter, & Joe Nese (2023). `r embed_file_quarto(path = here::here("nopublish", "doi_0117700131644221148122.pdf"), text = "Equating oral reading fluency scores: A model-based approach.")` Educational and Psychological Measurement._Educational and Psychological mMeasurement, 84_, 190-209.**
</p>
...
<p style="text-indent: -36px; padding-left: 36px; color:#6699AC;">
**Zhongjie Wu, Eric C. Larson, Makoto Sano, Doris Baker, Nathan Gage, & Akihito Kamata, (2023, July). `r embed_file_quarto(path = here::here("nopublish", "acmlas_2023.pdf"), text = "Towards Scalable Vocabulary Acquisition Assessment with BERT.")` ACM International Conference on Learning at Scale (L@S), Copenhagen, Denmark.**
</p>
...
**Kuo Wang, Xin Qiao, George Sammit, Eric C. Larson, Joe Nese, and Akihito Kamata (2023, April). `r embed_file_quarto(path = here::here("nopublish", "KuoWang_NCME2023_FinalPaper.pdf"), text = "Improving Automated Scoring of Prosody Using Deep Learning Algorithm.")` Presented at National Council for Measurement in Education (NCME), Chicago, IL.**

</p>

<font size="-0.5">

> This study proposed and evaluated an improved approach for automated scoring for prosody in oral reading fluency assessment by focusing on specific prosodic features and including the samples from text-to-speech (TTS) audio in the training. This research explored different models by using deep learning with different prosodic and frequency-related acoustic features to produce better performance, especially in cross-domain validation, where phrases are assumed to be unknown. The results demonstrated that the model with selected prosodic features had better cross-domain performance with an accuracy of 59.4% compared to 50.4% of the previous research. Our finding also demonstrated that the students have more weight to influence the estimating performance of cross-domain validation compared to the passages.
...
<p style="text-indent: -36px; padding-left: 36px; color:#6699AC;">
**Xin Qiao, Akihito Kamata, Eric C. Larson, Kuo Wang, and Srunya Somsong (2023, April). Detailed Prosody Report for Oral Reading Fluency Assessment via Machine Learning. Presented at National Council for Measurement in Education (NCME), Chicago, IL.**
</p>
...
<p style="text-indent: -36px; padding-left: 36px; color:#6699AC;">
**Joe Nese (2022). [*A Summary of An Effort to Improve the Practical and Psychometric Properties of Traditional ORF.*](https://raw.githubusercontent.com/jnese/SSSR_2022/main/scripts/sssr_2022_jftnese.pdf) Society for the Scientific Study of Reading (SSSR), Newport Beach, CA.**
</p> 

<font size="-0.5">

> Oral reading fluency (ORF) is an essential part of reading proficiency, and curriculum-based measurement of oral reading fluency is perhaps the most prevalent reading assessment used in classrooms across the U.S. CORE is a computerized ORF assessment framework that uses a scoring algorithm based on automatic speech recognition (ASR) and a latent variable psychometric model to produce model-based ORF words scores (words correct per minute [WCPM]). The purpose of this paper is to review the findings of CORE, a project developed to improve some of the practical and psychometric properties of traditional ORF assessment, including: administration errors, opportunity cost, passage inequivalence, score reliability and precision, and consequential validity. The first study ([Nese & Kamata, 2021](https://psycnet.apa.org/record/2020-81606-001)) applied mixed-effect models to estimate differences in ORF scores and reported evidence to support ASR scoring (compared to human scoring) and shorter ORF passages (comparable to traditional ORF passages). The second study ([Nese & Kamata, 2020](https://journals.sagepub.com/doi/pdf/10.1177/1534508420937801?casa_token=T2NeSfs7Pl0AAAAA:mbkSLGb8B9GFSieAGVBeJmcIUaXipIOeDJLX9KquuQiPqMXeiOkfnSMNxL7x1ojQGS5IYylPEpdu0Q)) evaluated 4,084 ORF scores from 1,021 students in and found that the conditional SEMs of model-based WCPM estimates ([Kara et al., 2020](https://journals.sagepub.com/doi/pdf/10.1177/0013164419900208?casa_token=EdF1QvSGMsUAAAAA:GXoTPkfRb9OMCoIK5ls9r9E6UDGzk1V3HTFD1eTimQq_fBYFimEZTDla2vGfihz27pNK6YDZeQ7YSg)) were substantially smaller than the reported SEMs of traditional ORF systems, especially for scores below the 25th percentile. The third study ([Nese, 2022](https://journals.sagepub.com/doi/pdf/10.1177/23328584211071112)) compared CORE model-based WCPM scores to traditional ORF WCPM and found that CORE had better within-growth properties, and better predictive performance, providing preliminary evidence that CORE can be used for consequential assessment. We discuss these findings in the context of the current state and future advances in oral reading fluency assessment.
...
<p style="text-indent: -36px; padding-left: 36px; color:#6699AC;">
**Nese, J. F. T., & Kamata, A. (2021). `r embed_file_quarto(path = here::here("nopublish", "doi_10137spq0000415.pdf"), text = "Evidence for automated scoring and shorter passages of CBM-R in early elementary school.")` *School Psychology, 36*, 47-59.**</p>
...
<p style="text-indent: -36px; padding-left: 36px; color:#6699AC;">
**Nese, J. F. T., & Kamata, A. (2021). `r embed_file_quarto(path = here::here("nopublish", "doi_1011771534508420937801.pdf"), text = "Addressing the large standard error of traditional CBM-R: Estimating the conditional standard error of a model-based estimate of CBM-R.")` *Assessment for Effective Intervention, 47*, 53-58.**</p>
...
<p style="text-indent: -36px; padding-left: 36px; color:#6699AC;">
**Kara, Y., Kamata, A., Potgieter, C., & Nese, J. F. (2020). `r embed_file_quarto(path = here::here("nopublish", "doi_1011770013164419900208.pdf"), text = "Estimating model-based oral reading fluency: A Bayesian approach.")` *Educational and Psychological Measurement, 80*, 847-869.**</p>
...
<p style="text-indent: -36px; padding-left: 36px; color:#6699AC;">
**Kamata, A., Kara, Y., Potgieter, C. J., & Nese, J. F. T. (2020, March). *Equating oral reading fluency scores: A model-based approach.* Paper accepted for presentation at the 8th annual Texas Universities Educational Statistics and Psychometrics Meeting, College Station, TX. (Conference Canceled)**</p>
